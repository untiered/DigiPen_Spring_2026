...
Purpose: This file contains the implementation for a binary spatial partition tree.
...
bool BspTree::RayCast(const Ray& ray, float& t, float planeEpsilon, float triExpansionEpsilon, int debuggingIndex)
{
	...
	if (((int)(Math::Abs(t) * 1000.0f + 1e-6f)) % 10 == 5
		&& ((int)(Math::Abs(t) * 10000.0f + 1e-6f)) % 10 == 0)
		t -= 0.001;
...
void BspTree::Intersection(BspTree* tree, float k, float epsilon)
{
	...
	// OR them together with Union
	Union(tree, k, epsilon);			// ~A|~B

	// then invert our tree again
	Invert();							// ~(~A|~B)
}
...
{
	/******Student:Assignment4******/
	if (m_root) Draw(m_root, 0, level, color, bitMask);
}
...
void BspTree::ClearTree(BSPNode *node)
{
	if (!node) return;
...
bool BspTree::RecursiveRayCast(BSPNode* node, const Ray& ray, float tMin, float tMax, float& t, float planeEpsilon, float triExpansionEpsilon)
{
	...
	if (RayPlane(ray.mStart, ray.mDirection, node->m_splitPlane.mData, tPlane)) // use the default epsilon value
	{
		...
		if (tPlane + t_e < tMin) return RayCast_Case_4(node, ray, tMin, tMax, t, planeEpsilon, triExpansionEpsilon, tPlane);
...
TriangleList BspTree::ClipTriangles(TriangleList &triangles, BSPNode *clippingNode, float epsilon)
{
	...
	if (clippingNode->m_left) // if left node exists send left
	{
		...
		// clip back triangles by not adding them to the sentLeft vector
	}
...
void BspTree::Draw(BSPNode* node, int depth, int level, const Vector4& color, int bitMask)
{
	if (level == -1 || depth == level)
	{
		for (Triangle const& tri : node->m_triangles)
		{
			DebugShape& debugShape = gDebugDrawer->DrawTriangle(tri);
			debugShape.Color(color);
			debugShape.SetMaskBit(bitMask);
		}
	}

	if (node->m_right) Draw(node->m_right, depth + 1, level, color, bitMask);
	if (node->m_left) Draw(node->m_left, depth + 1, level, color, bitMask);
}