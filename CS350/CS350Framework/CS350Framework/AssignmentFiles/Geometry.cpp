                                   ...
                                   const Vector3& aabbMin, const Vector3& aabbMax, size_t& lastAxis)
{
    ...
    // test last axis
    IntersectionType::Type it = PlaneAabb(planes[lastAxis], aabbMin, aabbMax);
    if (it == IntersectionType::Outside) return IntersectionType::Outside;
    if (it == IntersectionType::Overlaps) overlaps = true;

    for (int i = 0; i < 6; ++i)
    {
        if (i == lastAxis) continue; // dont re-test the last axis
        it = PlaneAabb(planes[i], aabbMin, aabbMax);
